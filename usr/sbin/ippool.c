...
/*                                                                        */
/* 53ipfl53H src/ipfl/usr/sbin/ippool.c 1.2                               */
/*                                                                        */
...
/*                                                                        */
/* IBM_PROLOG_END_TAG                                                     */
static char sccsid[] = "@(#)71  1.2  src/ipfl/usr/sbin/ippool.c, ipflt, 53ipfl53H, 0624A_53ipfl53H 6/13/06 18:41:05";
/*
...
char *argv[];
{
	int err;
        ...
        {
                struct stat stbuf;
                if (!stat("/usr/sbin/ippool64", &stbuf))
                {
                        exit(execv("/usr/sbin/ippool64", argv));
                }
                else
                {
                        fprintf(stderr,"%s : failed.  The kernel is running in 64-bit mode, but %s cannot be found\n",argv[0], "/usr/sbin/ippool64");
                        exit(1);
                }
        }
#else /* __64BIT__ */
