...
/*                                                                        */
/* 53ipfl53H src/ipfl/usr/sbin/ipscan_y.y 1.3                             */
/*                                                                        */
...
/*                                                                        */
/* IBM_PROLOG_END_TAG                                                     */
/* @(#)73  1.3  src/ipfl/usr/sbin/ipscan_y.y, ipflt, 53ipfl53H, 0624A_53ipfl53H 6/13/06 18:41:08 */
%{
#include <sys/types.h>
#include <sys/ioctl.h>
#include "ipf.h"
...
char *argv[];
{
	...
	int c;
        ...
        {
                struct stat stbuf;
                if (!stat("/usr/sbin/ipscan64", &stbuf))
                {
                        exit(execv("/usr/sbin/ipscan64", argv));
                }
                else
                {
                        fprintf(stderr,"%s : failed.  The kernel is running in 64-bit mode, but %s cannot be found\n",argv[0], "/usr/sbin/ipscan64");
                        exit(1);
                }
        }
#else /* __64BIT__ */
