...
/*                                                                        */
/* 53ipfl53H src/ipfl/usr/sbin/ipfs.c 1.2                                 */
/*                                                                        */
...
/*                                                                        */
/* IBM_PROLOG_END_TAG                                                     */
static char sccsid[] = "@(#)64  1.2  src/ipfl/usr/sbin/ipfs.c, ipflt, 53ipfl53H, 0624A_53ipfl53H 6/13/06 18:40:59";
/*
...
char *argv[];
{
	...
	char *dirname = NULL, *filename = NULL, *ifs = NULL;
        ...
        {
                struct stat stbuf;
                if (!stat("/usr/sbin/ipfs64", &stbuf))
                {
                        exit(execv("/usr/sbin/ipfs64", argv));
                }
                else
                {
                        fprintf(stderr, "%s : failed.  The kernel is running in 64-bit mode, but %s cannot be found\n",argv[0], "/usr/sbin/ipfs64");
                        exit(1);
                }
        }
#else /* __64BIT__ */
