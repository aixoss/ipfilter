...
/*                                                                        */
/* 53ipfl53H src/ipfl/usr/sbin/ipfstat.c 1.3                              */
/*                                                                        */
...
/*                                                                        */
/* IBM_PROLOG_END_TAG                                                     */
static char sccsid[] = "@(#)65  1.3  src/ipfl/usr/sbin/ipfstat.c, ipflt, 53ipfl53H, 0624A_53ipfl53H 6/13/06 18:41:04";
/*
...
char *argv[];
{
	...
	u_32_t frf;
        ...
        {
                struct stat stbuf;
                if (!stat("/usr/sbin/ipfstat64", &stbuf))
                {
                        exit(execv("/usr/sbin/ipfstat64", argv));
                }
                else
                {
                        fprintf(stderr,"%s : failed.  The kernel is running in 64-bit mode, but %s cannot be found\n",argv[0], "/usr/sbin/ipfstat64");
                        exit(1);
                }
        }
#else /* __64BIT__ */
